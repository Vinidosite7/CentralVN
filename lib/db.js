...
export async function addBill(b) {
  const { data, error } = await supabase.from("bills").insert({
    ...
    due_day: b.dueDay || null, last_generated: b.recurring ? (b.due ? b.due.slice(0,7) : null) : null,
  }).select().single();
  return { data, error };
}
...
export async function addExpense(e) {
  const { data, error } = await supabase.from("expenses").insert({
    date: e.date, title: e.title, amount: e.amount, cat: e.cat, method: e.method, platform: e.platform || null,
  }).select().single();
  return { data, error };
}
...
function mapLoanFromDb(l) {
  return {
    id: l.id, title: l.title, creditor: l.creditor, kind: l.kind,
    total: Number(l.total), installments: l.installments,
    paidInstallments: l.paid_installments, paidAmount: Number(l.paid_amount),
    due: l.due, cat: l.cat, settled: l.settled,
    person: l.person || l.creditor || "Sem nome",
    direction: l.direction || "owe", // 'owe' = eu devo | 'owed' = me devem
  };
}
export async function loadLoans() {
  const { data } = await supabase.from("loans").select("*").eq("settled", false).order("due", { ascending: true, nullsFirst: false });
  return (data || []).map(mapLoanFromDb);
}
export async function addLoan(l) {
  const { data, error } = await supabase.from("loans").insert({
    title: l.title, creditor: l.creditor || l.person, kind: l.kind, total: l.total,
    installments: l.installments, paid_installments: 0, paid_amount: 0,
    due: l.due || null, cat: l.cat || "Pessoal", settled: false,
    person: l.person || "Sem nome", direction: l.direction || "owe",
  }).select().single();
  return { data: data ? mapLoanFromDb(data) : null, error };
}
export async function updateLoan(id, patch) {
  ...
  if (patch.installments != null) row.installments = patch.installments;
  if (patch.due !== undefined) row.due = patch.due || null;
  if (patch.person != null) row.person = patch.person;
  if (patch.direction != null) row.direction = patch.direction;
  if (patch.cat != null) row.cat = patch.cat;
  if (patch.paidAmount != null) row.paid_amount = patch.paidAmount;
  if (patch.paidInstallments != null) row.paid_installments = patch.paidInstallments;
  if (patch.settled != null) row.settled = patch.settled;
  return supabase.from("loans").update(row).eq("id", id);
}
export async function delLoan(id) {
  return supabase.from("loans").delete().eq("id", id);
}
// paga 1 parcela (parcelada) OU um valor (à vista); gera gasto automático
export async function payLoan(loan, amount) {
  const isParcelada = loan.kind === "parcelada";
  let patch, gastoValor, gastoTitulo;

  if (isParcelada) {
    const novaPaga = Math.min(loan.installments, loan.paidInstallments + 1);
    const valorParcela = loan.total / loan.installments;
    gastoValor = valorParcela;
    gastoTitulo = `${loan.title} (parcela ${novaPaga}/${loan.installments})`;
    patch = {
      paid_installments: novaPaga,
      paid_amount: loan.paidAmount + valorParcela,
      settled: novaPaga >= loan.installments,
    };
  } else {
    const add = amount || 0;
    const novoPago = Math.min(loan.total, loan.paidAmount + add);
    gastoValor = add;
    gastoTitulo = `${loan.title} (pagamento)`;
    patch = { paid_amount: novoPago, settled: novoPago >= loan.total };
  }

  const { error } = await supabase.from("loans").update(patch).eq("id", loan.id);
  // gera gasto automático SÓ quando EU devo (owe) — pagar sai do caixa.
  // Quando ME devem (owed), receber é entrada, então NÃO vira gasto.
  let gasto = null;
  if (!error && gastoValor > 0 && loan.direction !== "owed") {
    const { data } = await supabase.from("expenses").insert({
      date: new Date().toISOString().slice(0, 10),
      title: gastoTitulo, amount: gastoValor, cat: loan.cat || "Pessoal", method: "—",
    }).select().single();
    gasto = data;
  }
  return { error, patch, gasto };
}
