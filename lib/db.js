...
function mapPurchaseFromDb(p) {
  return { id: p.id, cardId: p.card_id, desc: p.descricao, total: Number(p.total), installments: p.installments, startMonth: p.start_month, paid: p.paid };
}
...
export async function addPurchase(p) {
  const { data, error } = await supabase.from("card_purchases").insert({
    ...
    installments: p.installments, start_month: p.startMonth, paid: p.paid || 0,
  }).select().single();
...
export async function addDebt(d) {
  const { data, error } = await supabase.from("debts").insert({
    who: d.who, amount: d.amount, direction: d.direction, note: d.note, settled: false,
  }).select().single();
  return { data, error };
}
